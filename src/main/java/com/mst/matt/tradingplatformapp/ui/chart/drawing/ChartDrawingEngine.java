...
public class ChartDrawingEngine {
    ...
    public void undo() {
        ...
        notifySelection();
        host.requestRender();
        // Notify persistence if available
    ...
    public boolean handleMouseDragged(MouseEvent e, DrawingRenderer.RenderContext ctx) {
        ...
        if (inProgress != null && !inProgress.getPoints().isEmpty()) {
            ...
            // For tools needing >1 point: update the preview of the second point during drag
            if (numPoints == 1 && inProgress.getToolType().requiredPoints() > 1) {
                inProgress.getPoints().add(pt);
            } else if (numPoints >= 2) {
                // Update the last point as live preview
    ...
    public void deleteSelected() {
        ...
        // Record DELETE for undo (before removing)
    ...
    /**
     * Begins in-place text editing for annotation tools (TEXT_LABEL, CALLOUT, NOTE_ICON).
     * For NOTE_ICON, opens a dialog for longer text.
     * For TEXT_LABEL and CALLOUT, activates inline editing mode.
     */
    ...
    /** Opens a TextInputDialog for NOTE_ICON with full multi-line support. */
    private void openNoteEditDialog(ChartDrawing d, DrawingHistoryManager.DrawingSnapshot before) {
        ...
        if (host.getWindow() != null) {
            try {
                dialog.initOwner(host.getWindow());
            } catch (Exception ignored) {}
        }
        dialog.showAndWait().ifPresent(newText -> {
            d.getProperties().setText(newText);
            // Record MODIFY for undo
    ...
    /**
     * Activates inline text editing for TEXT_LABEL / CALLOUT.
     * Uses a floating TextField overlay or a simple dialog, depending on availability.
     * Falls back to TextInputDialog for simplicity in the current canvas-based architecture.
     */
    private void openInlineTextEditor(ChartDrawing d, DrawingRenderer.RenderContext ctx,
        ...
        String current = d.getProperties().getText() != null ? d.getProperties().getText() : "";
        // Set editing flag for visual cue
    ...
    private void finalizeInProgress() {
        ...
        updatePositionPricesFromAnchors(inProgress);
        // Default text for annotation tools
    ...
    private boolean isNearRect(ChartDrawing d, double x, double y, DrawingRenderer.RenderContext ctx) {
        ...
        return Math.abs(x - minX) < HIT_THRESHOLD || Math.abs(x - maxX) < HIT_THRESHOLD
                ...
                || (x >= minX && x <= maxX && y >= minY && y <= maxY); // inside fill
    }
    ...
    private boolean isNearEllipse(ChartDrawing d, double x, double y, DrawingRenderer.RenderContext ctx) {
        ...
        return val <= 1.2; // inside or on ellipse (with tolerance)
    }
    ...
    private void showContextMenu(double screenX, double screenY, ChartDrawing d) {
        ...
        // Edit text option for annotations
