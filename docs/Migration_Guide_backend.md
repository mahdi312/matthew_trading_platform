...
## Part 0b — Progress Review: `TradingPlatformApp_V_2_0_5` (Steps 8–12, pre-Step 13)

*Added after reviewing the codebase with Steps 8–12 attempted, Step 13 not yet started. All three Part 0
gaps (market-service wiring, ai-service OHLCV client, charting controllers) are now confirmed fixed — good
work. This section covers five new findings from a deeper pass specifically checking caching, logging,
transactional correctness, threading, and abstraction-layer consistency across the newly added code.*

**Confirmed fixed from Part 0:** Gap 1 (`MarketDataController` now calls `OhlcvCacheService`, which is a
genuinely well-built two-tier Caffeine→Redis→registry cache with graceful Redis-failure degradation), Gap 2
(`ai-service` now has `MarketServiceOhlcvProvider` delegating to `market-service` via `MarketDataClient`,
registered ahead of the NoOp fallback), Gap 3 (`ChartingController`/`IndicatorController` now exist and
correctly resolve `userId` the same way `alert-service` does). Redis is wired into `market-service`
correctly. Kafka trade-event publishing (`trades.executed`/`trades.closed`) now exists in `trading-service`
and matches the topic names `notification-service`'s `TradeEventConsumer` already expects — good
cross-service naming discipline.

### 1. Caching — one service repeats the exact Gap-1 mistake

`reference-data-service` got a genuinely well-designed two-tier cache
(`cache/RefDataCacheService.java` + `config/RefDataCacheConfig.java`, with sensible per-domain TTLs — news
120s/10min, fundamentals 300s/60min, sentiment 60s/5min). **But `ReferenceDataController` never calls it —
it still only imports `ProviderRegistry` and calls providers directly, the same way it did before caching
existed.** `RefDataCacheService` is referenced by nothing except its own config class. Right now,
`reference-data-service` hits AlphaVantage/Finnhub/CoinGecko/etc. on every single request with zero cache
protection — exactly the orphaned-abstraction pattern from Gap 1, recurring in a second service.

**Prompt:**

> In `services/reference-data-service`, wire `RefDataCacheService` into `ReferenceDataController`: every
> endpoint (`/fundamentals/{symbol}`, `/news`, `/sentiment`, `/calendar`, `/search`, `/nft/collections`,
> `/defi/pools`) should call the matching `RefDataCacheService` method (checking L1 → L2 → calling
> `ProviderRegistry.executeWithFallback()` only on a full miss, writing back to both tiers), instead of
> calling `ProviderRegistry` directly. Use `OhlcvCacheService` in `market-service` as the reference pattern
> for how the controller-to-cache-to-registry chain should look.

*(`ai-service` doesn't need its own provider-level cache — it delegates OHLCV/news to services that already
cache. `trading-service`/`identity-service`/`notification-service`/`alert-service` are DB-CRUD or
message-passing services, not repeated-external-read services, so they don't need this pattern.)*

### 2. Logging — per-class logging is good; there's no cross-service correlation

Every class sampled in this review (`OhlcvCacheService`, `AlertTriggeredEventConsumer`,
`TradingOrchestrationService`, `EmailNotificationChannel`, etc.) uses `@Slf4j` with sensible log
levels and useful context in the message (IDs, symbols, cache tier hit/miss). That part is genuinely good.
But: no service has a `logback-spring.xml` (only the legacy `desktop/` module does), nothing sets a
correlation/request ID at the Gateway and propagates it through Feign calls and Kafka headers, and there's
no Micrometer/OpenTelemetry dependency anywhere — despite the architecture doc explicitly calling for both
("*Standardize correlation IDs and business IDs*" and "*Micrometer with OpenTelemetry for metrics and
tracing*"). Today, if a single user request touches `market-service` → `ai-service` (Feign) →
`reference-data-service` (Feign), there is no shared ID to grep across all three services' logs to
reconstruct that one request's path — you can only correlate by eye by matching `userId`/`symbol`/timestamp.

**Prompt:**

> Add a correlation-ID filter to `infra/gateway-service`: generate (or forward, if the client already sent
> one) an `X-Correlation-Id` header on every request, and add an MDC-populating filter to every downstream
> service (a shared filter class in `shared/contracts` or a small new `shared/observability` module, so it
> isn't reimplemented seven times) that reads that header, puts it in SLF4J's MDC, and clears it after the
> request. Update `application.yml` `logging.pattern.*` in every service to include `%X{correlationId}`.
> Propagate the header through every `@FeignClient` call (a Feign `RequestInterceptor` reading from MDC) and
> attach it as a Kafka header on every `KafkaTemplate.send()` call (`TradeEventPublisher`,
> `AlertEventPublisher`), reading it back into MDC in the `@KafkaListener` methods that consume them. This
> is infrastructure, not business logic — one shared filter/interceptor pair reused everywhere, not
> per-service code.
>
> Separately (can be a later pass): add `micrometer-tracing-bridge-otel` + `spring-boot-starter-actuator` to
> every service and point export at whatever you choose to run locally (Zipkin/Tempo/Jaeger via
> docker-compose) — this gives you the same correlation automatically plus latency breakdowns, and the
> manual MDC filter above still helps in the meantime since full OTel wiring is a bigger lift than one
> prompt.

### 3. Transactional correctness — a real bug in `TradingOrchestrationService`

`TradingOrchestrationService.placeTrade()` is annotated `@Transactional` at the method that also calls
`placeLiveBrokerTrade()`, which does two things in this order, **inside that one transaction**:

1. `provider.placeFuturesOrder(...)` / `placeSpotOrder(...)` — a **live network call to the BitUnix
   exchange API**.
2. `tradeService.saveTrade(trade)` — the DB write.

This holds a DB connection/transaction open for the full duration of an external network call, which is a
real risk under load (connection-pool exhaustion if BitUnix is slow) and a real correctness problem: **if
the broker call succeeds and the subsequent DB save then fails and rolls back, you have an order placed on
the live exchange with no local record of it at all.** A `@Transactional` boundary can roll back your
database; it cannot roll back an order that already executed on BitUnix. This is exactly the class of issue
Step 13 (event/idempotency audit, not yet started) exists to catch — this is a concrete instance of it.

**Prompt:**

> In `TradingOrchestrationService`, remove `@Transactional` from `placeTrade()`. Restructure
> `placeLiveBrokerTrade()` so the broker call happens with no transaction open, and only the subsequent
> `tradeService.saveTrade(...)` runs inside a transaction (keep `@Transactional` on `TradeService.saveTrade`
> itself, or add a narrowly-scoped `@Transactional` only around the save). Add explicit handling for the
> case where `provider.placeFuturesOrder`/`placeSpotOrder` succeeds but the subsequent save throws: at
> minimum, log a clearly-flagged error with the broker's returned order ID so it can be manually
> reconciled, since the money has already moved regardless of what your database says. This is also the
> right place to start the reconciliation-job stub already scoped in Step 13.

### 4. Multi-threading — one synchronous chain that can stall Kafka consumption

`notification-service`'s `AlertTriggeredEventConsumer`/`TradeEventConsumer` run on a single-concurrency
Kafka listener thread (documented as intentional for dev — reasonable) and call every `NotificationChannel`
synchronously in a loop, including `EmailNotificationChannel` (blocking `JavaMailSender.send()` — real SMTP
I/O) and `TelegramNotificationChannel` (blocking Bot API HTTP call). Neither uses `@Async`. A slow/hanging
SMTP or Telegram call blocks the Kafka listener thread entirely — with concurrency=1, that means **all**
alert/trade notifications queue up behind one slow send, and a sufficiently slow provider risks exceeding
`max.poll.interval.ms` and triggering a consumer-group rebalance.

Elsewhere, threading is handled well: `market-service`'s `MarketDataSyncService` correctly declares
`@EnableAsync`/`@EnableScheduling` and uses `@Async` for the sync path; the BitUnix WS clients (both
`market-service` and `trading-service`) use dedicated connections as before. No service defines an explicit
bounded `ThreadPoolTaskExecutor` — everything relies on Spring Boot's auto-configured default, which is
core-8/unbounded-queue and fine for current scale, but worth hardening explicitly before production load
(a queue that can grow unbounded under sustained overload is a slow-motion memory problem, not an immediate
one).

**Prompt:**

> In `notification-service`, mark each `NotificationChannel.send()` implementation (or wrap the calls in
> `AlertTriggeredEventConsumer`/`TradeEventConsumer`) with `@Async`, backed by a dedicated bounded
> `ThreadPoolTaskExecutor` bean (e.g. `notificationSendExecutor`, core/max sized for expected channel count,
> bounded queue with a `CallerRunsPolicy` rejection handler so overload degrades rather than silently drops).
> Add `@EnableAsync` to `NotificationServiceApplication` if not already present. Confirm the Kafka listener
> method itself returns quickly once the async dispatch is fired (don't `.get()`/block on the async result
> inside the listener — that would defeat the purpose). While you're in this area, define an explicit
> `ThreadPoolTaskExecutor` bean (rather than relying on the Spring Boot default) in any other service using
> `@Async` (`market-service`), with a bounded queue capacity and a named thread prefix for readable thread
> dumps/logs.

### 5. Abstraction-layer consistency — one cross-service contract was left half-built

`shared/contracts`' `UserPreferencesDto` — the DTO every `NotificationChannel` receives — is explicitly
documented in its own Javadoc as *"owned conceptually by identity-service, as part of the user's profile."*
Step 9 built `identity-service`'s `UserProfile`/`AppSetting` models, but **neither has an email/Telegram/
in-app notification-preference field**, and `notification-service`'s `AlertTriggeredEventConsumer.
resolvePreferences()` is still exactly the hardcoded stub it was before Step 9 existed (its own comment
still says "TODO (Step 9): Replace this stub with a Feign call to identity-service" — Step 9 shipped, this
particular follow-up didn't). Concretely: the stub always returns `email = null`, and
`EmailNotificationChannel.send()` bails out whenever `email` is blank — **so real emails never actually
send today**, regardless of the SMTP wiring being otherwise correct, because the one piece of data that
would let it send was never connected. Aside from this one contract, every other abstraction layer checked
this round (`OhlcvDataProvider` chain, `NotificationChannel` chain, `ProviderRegistry` usage in the new
`reference-data-service` and `ai-service` code) is registered and named consistently with what's already
established — this is the only place the pattern broke down.

**Prompt:**

> Add notification-preference fields to `identity-service`'s `UserProfile` (or a new
> `NotificationPreferences` child entity — your call, but keep it under the same `userId`):
> `inAppEnabled`, `emailEnabled`, `email` (can default to the account email), `telegramEnabled`,
> `telegramChatId`. Add `GET /api/profile/{userId}/preferences` to `ProfileController`, returning
> `UserPreferencesDto` from `shared/contracts` directly (it already has the exact shape needed — don't
> create a parallel DTO). In `notification-service`, add an `IdentityClient` (Feign, same pattern as
> `ai-service`'s `MarketDataClient`) and replace `AlertTriggeredEventConsumer.resolvePreferences()`'s
> hardcoded stub with a real call to this new endpoint. Do the same for `TradeEventConsumer` if it has an
> equivalent stub. Add a sensible fallback (e.g. `@FeignClient(fallback = ...)`, matching the pattern
> `trading-service`'s `FundamentalsClientFallback` already established) so a down `identity-service` doesn't
> take out notification delivery entirely — degrade to in-app-only or log-and-skip, not a thrown exception.
...
| # | Step | Depends on | Priority | Status (V2.0.5) |
|---|---|---|---|---|
| 1 | Trading Service — trades/portfolio/API | — | P0 | ✅ Done |
| 2 | Market Service — OHLCV storage & symbols | — | P0 | ✅ Done |
| 3 | Market Service — multi-provider OHLCV | Step 2 | P0 | ✅ Done |
| 3b | Wire Gap 1 | Step 3 | P0 | ✅ Done — confirmed wired |
...
| 6 | AI Service — analysis/news synthesis | Step 4 (calls NewsProvider) | P0 | ✅ Done |
| 6b | Wire Gap 2 | Step 6 | P0 | ✅ Done — confirmed wired |
| 7 | Market Service — charting/indicators | Step 2 (needs OHLCV) | P0 | ✅ Done |
| 7b | Wire Gap 3 | Step 7 | P0 | ✅ Done — controllers exist, `userId` resolved correctly |
| — | Redis L2 cache | Step 3b, Step 4 | P1 | ⚠️ Done for `market-service`; built-but-not-wired for `reference-data-service` — Part 0b §1 |
| 8 | Notification Service — real send | Step 1 (trade events) | P0 | ⚠️ SMTP/Telegram real, but never actually fires — Part 0b §5 |
| 9 | Identity Service — profile/settings/admin | — | P1 | ⚠️ Profile/admin done; notification-preferences sub-piece missing — Part 0b §5 |
| 10 | Trading Service — reporting/export | Step 1, Step 4 | P1 | ✅ Done |
| 11 | Gateway routes | Steps 1–10 | P0 (do right after each service gets its controller) | ✅ Done — every service routed |
| 12 | Desktop client shrink | Steps 1–11 | P1 | 🔄 In progress (Login/MainDashboard/Register/TradeEntry done; explicitly, honestly tagged transitional in `pom.xml`) |
| 13 | Event/idempotency audit | Step 1, Step 8 | P1 | Not started — Part 0b §3 found a concrete instance (`TradingOrchestrationService`) to fix as part of this step |
| 14 | Postgres-per-service | All above | P2 | Not started (all services still on H2) |
| — | Reference-data cache wiring | — | **P0 — do next** | Not started — Part 0b §1 |
| — | Notification-preferences (identity ↔ notification) | Step 9 | **P0 — do next** | Not started — Part 0b §5 |
| — | Correlation-ID logging | — | P1 | Not started — Part 0b §2 |
| — | Async notification dispatch | Step 8 | P1 | Not started — Part 0b §4 |

**Do next, in this order:** reference-data cache wiring → notification-preferences wiring (these two make
existing "done" features actually work) → fix `TradingOrchestrationService`'s transaction boundary (real
bug, do before more trade volume goes through it) → async notification dispatch → correlation-ID logging →
finish Step 13 (idempotency + outbox, now with a concrete first target) → Step 12 (continue domain-by-domain)
→ Step 14.
