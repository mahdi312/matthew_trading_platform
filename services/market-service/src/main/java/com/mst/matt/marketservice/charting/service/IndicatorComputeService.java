...
import com.mst.matt.marketservice.charting.model.IndicatorDefinition;
import com.mst.matt.marketservice.charting.model.IndicatorDefinition.PriceSource;
 ...
 *
 * Ported from desktop {@code service.analysis.IndicatorComputeService}.
 * Adapted for ta4j 0.16 API (no MACD.getSignalLine/getHistogram — computed manually).
 */
@Service
public class IndicatorComputeService {
    ...
    public BarSeries toBarSeries(List<OhlcvBar> bars, String name) {
        ...
        for (OhlcvBar bar : bars) {
            ...
            ZonedDateTime closeZdt = openZdt.plus(period);
            try {
                series.addBar(period, closeZdt,
                ...
                log.trace("Skipping bar at {}: {}", bar.getOpenTime(), e.getMessage());
            }
        }
        return series;
    }
    ...
    private void computeParabolicSar(IndicatorDefinition def, BarSeries s) {
        def.setSeries(extract(new ParabolicSarIndicator(s,
                DecimalNum.valueOf(0.02), DecimalNum.valueOf(0.2)), s));
    }
    ...
    private void computeMacd(IndicatorDefinition def, BarSeries s) {
        ...
        int signalPer  = def.getPeriod3() > 0 ? def.getPeriod3() : 9;
        var src        = priceIndicator(s, def.getPriceSource());
        var macd       = new MACDIndicator(src, fast, slow);
        // ta4j 0.16: no getSignalLine()/getHistogram() — compute manually
        var sig        = new EMAIndicator(macd, signalPer);
        var hist       = NumericIndicator.of(macd).minus(NumericIndicator.of(sig));
    ...
    private void computeAroon(IndicatorDefinition def, BarSeries s) {
        int p   = Math.max(2, def.getPeriod());
        // ta4j 0.16: AroonOscillatorIndicator exposes getAroonUpIndicator()/getAroonDownIndicator()
        var osc  = new AroonOscillatorIndicator(s, p);
        var up   = osc.getAroonUpIndicator();
        var down = osc.getAroonDownIndicator();
    ...
    private void computeTwoMa(IndicatorDefinition def, BarSeries s) {
        ...
        var src  = priceIndicator(s, def.getPriceSource());
        def.setSeries(extract(new EMAIndicator(src, fast), s));
        def.putExtraSeries("line2", extract(new EMAIndicator(src, slow), s));
        def.autoLabel();
    }
    ...
    private void computeThreeMa(IndicatorDefinition def, BarSeries s) {
        ...
        var src  = priceIndicator(s, def.getPriceSource());
        def.setSeries(extract(new EMAIndicator(src, fast), s));
        def.putExtraSeries("line2", extract(new EMAIndicator(src, mid),  s));
        def.putExtraSeries("line3", extract(new EMAIndicator(src, slow), s));
        def.autoLabel();
    }
}
