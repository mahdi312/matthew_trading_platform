...
import com.mst.matt.marketservice.model.SymbolEntry.AssetType;
...
public interface SymbolEntryRepository extends JpaRepository<SymbolEntry, Long> {
    ...
    @Query("SELECT s FROM SymbolEntry s WHERE s.assetType = :type AND (UPPER(s.symbol) LIKE UPPER(CONCAT('%',:q,'%')) OR UPPER(s.name) LIKE UPPER(CONCAT('%',:q,'%'))) ORDER BY s.symbol")
    List<SymbolEntry> search(@Param("type") AssetType type, @Param("q") String query);
    @Query("SELECT s FROM SymbolEntry s WHERE (UPPER(s.symbol) LIKE UPPER(CONCAT('%',:q,'%')) OR UPPER(s.name) LIKE UPPER(CONCAT('%',:q,'%'))) ORDER BY s.assetType, s.symbol")
