...
/**
 * Registry of LLM models available in the AI News &amp; Insights tab.
 * Models are shown in the UI picker; only models with a configured API key are usable.
 */
@Component
public class AiLlmModelRegistry {
    ...
    private static final List<AiLlmModel> ALL_MODELS = List.of(
            ...
            // Cerebras — see https://api.cerebras.ai/public/v1/models for current IDs
    ...
    @Value("${app.api.cerebras.key:}")
    private String cerebrasKey;

    @Value("${app.api.deepseek.key:}")
    private String deepSeekKey;

    @Value("${app.api.anthropic.key:}")
    private String anthropicKey;

    @Value("${app.api.openai.model:gpt-4o-mini}")
    private String defaultOpenAiModel;

    @Value("${app.api.groq.model:}")
    private String groqModelOverride;

    @Value("${app.api.cerebras.model:}")
    private String cerebrasModelOverride;

    @Value("${app.api.deepseek.model:}")
    private String deepSeekModelOverride;

    @Value("${app.api.anthropic.model:}")
    private String anthropicModelOverride;

    public List<AiLlmModel> allModels() {
        return ALL_MODELS;
    }
    ...
    /** Applies optional {@code app.api.<provider>.model} override from properties. */
    public AiLlmModel effectiveModel(AiLlmModel model) {
        String override = normalizeModelOverride(model.apiKeyProperty(), providerModelOverride(model.apiKeyProperty()));
        if (override == null || override.isBlank() || override.equals(model.modelId())) {
            return model;
        }
        return new AiLlmModel(
                model.id(),
                model.providerName(),
                model.displayName(),
                override.trim(),
                model.baseUrl(),
                model.format(),
                model.apiKeyProperty());
    }

    private static String normalizeModelOverride(String provider, String override) {
        if (override == null || override.isBlank()) return override;
        if ("groq".equals(provider) && "llama-3.3-70b".equals(override)) {
            return "llama-3.3-70b-versatile";
        }
        if ("cerebras".equals(provider) && "llama-3.3-70b".equals(override)) {
            return "gpt-oss-120b";
        }
        return override;
    }
    ...
    private String providerModelOverride(String apiKeyProperty) {
        return switch (apiKeyProperty) {
